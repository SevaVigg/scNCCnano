...
greyCols	<- grey.colors( length( unique( seuratObj@ident)))

heatmapPlotDir 	<- file.path(resolDir, "HeatMaps")
dir.create(heatmapPlotDir, showWarnings = FALSE)

IP_clust 	<- seuratObj@ident[names(curves_IP)]
...
MC_clust 	<- seuratObj@ident[names(curves_MC)]
...
CL3_clust 	<- seuratObj@ident[names(curves_CL3)]
